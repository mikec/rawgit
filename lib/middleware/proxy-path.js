...
module.exports = function (rootUrl) {
    ...
    return function (req, res, next) {
        ...
        var subdomainRoute = new Route(':username.:repo.:sha');
        var host = req.get('host').replace('.rawgit.dev:5000', '');
        var routeParams = subdomainRoute.match(host);
        var path = pathFromRequest(req, routeParams);
        ...
        https.get(params).on('response', function (incoming) {
...
function pathFromRequest(req, routeParams) {
